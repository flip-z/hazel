...
## 2026-02-09: Add "Run tick" to UI

- User request: include single-tick run functionality in the UI.
- Change applied: board top bar now has a `Run tick` button that triggers the same `hazel run` behavior (dispatch one READY task, run agent_command, move task to REVIEW) asynchronously.
